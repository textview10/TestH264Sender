...
public class CameraUtils {
    ...
    private static void setOrientation(CameraData cameraData, boolean isLandscape, Camera camera) {
        ...
        //  FIXME  xu.wang 暂时解决lenovo某款设备直播角度偏转的问题...
        if (TextUtils.equals(Build.MODEL, "Lenovo TB3-X70F")) {
            orientation += 90;
        }
        // FIXME xu.wang 暂时解决孙哥的,小米4c,MIUI欧洲版,横屏时角度不为0,
        // FIXME 导致相机初始化失败的问题
        if (isLandscape && TextUtils.equals(Build.MODEL,"Mi-4c") && orientation != 0){
            orientation = 0;
        }
